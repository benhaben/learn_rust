...
# 人话
滑动窗口用最近 L 根预测下一根：窗口右端必须停在 t，标签在 t 之后。
若窗口里含了标签当天，或用整段序列的 μ/σ 标准化，或把窗口当图片打乱，都是泄漏。
换成 LSTM / Transformer 不会自动免疫。

# 目的
序列模型照样会偷看未来。正确窗口的最后一个数是 y 的前一根，不是 y 自己。

# 场景
任何 RNN/CNN/Transformer 的滑动窗口特征、序列标准化。金融行是时间，禁止 shuffle。
