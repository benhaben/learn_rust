...
# 人话
时钟 bar = 按墙钟切（每分钟一根），冷清和热闹各占一根，信息不匀。
dollar bar = 成交额（价×量）凑满 N 元才收一根：活跃时出得勤，安静时稀。
同类还有 tick bar（每 N 笔）、volume bar（每 N 股）。

# 目的
按「信息到达」采样，波动/相关不被时钟扭曲。不是「趋势里更准」，只换尺子。

# 场景
tick/高频研究、波动估计。训练和实盘必须同一套 bar。阈值应事先定或只用过去估。
