...
# 人话
你试了 N 组参数，只报最大的那个 Sharpe。即使每条策略都是噪声（真 Sharpe=0），
「N 次里的最大值」期望也大于 0，N 越大越吹。这不是作弊代码，是多重检验。
Deflated Sharpe = 把零假设从「Sharpe=0」抬到「N 次里最大的期望」，再看还显著不显著。

# 目的
搜参扫出来的 Sharpe 2 要打折，避免把幸运噪声当圣杯。

# 场景
网格搜参、因子挖掘、只报冠军回测。持有集（最终只碰一次的样本）不能再拿来调参。
