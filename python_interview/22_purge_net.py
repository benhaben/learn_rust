...
# 人话
把三件事接成一条流水线：14 打三重屏障标签 → 08 按时间切并洗重叠 → 10 扣开平仓成本。
「永远预测 +1」可以有一个准确率；同一批事件都做多、扣双边费，净收益经常是负的。

# 目的
研究不能停在 accuracy。标签窗口重叠要 purge；测试结束后要 embargo；评估看扣费后的钱。

# 场景
事件驱动策略的验证闭环。仍是假价格，换真 px 即可，切分和扣费规则不变。
