...
# 人话
简单收益 R = 今/昨 − 1，就是涨了百分之几，钱的百分比。
对数收益 r = ln(今) − ln(昨)，就是价格的一阶对数差分（差分=相减，见 16）。
小涨跌时两者几乎一样：ln(1+x) ≈ x。

# 目的
钱可加 → 组合必须用简单收益加权。
多期可加 → 统计、累加用对数：r1+r2+… = ln(末价/首价)。简单收益只能连乘还原。

# 场景
回测净值、仓位加权、换手成本 → 简单 R，净值 (1+R).cumprod()。
多日累计、建模、近似正态 → 对数 r。不要把日对数加权当组合收益。
